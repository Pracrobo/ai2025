...
def create_vector_db():
    ...
    store = Chroma.from_documents(texts, embeddings, collection_name="travel", persist_directory=PERSIST_DIR) # DB 저장
    return store


def load_vector_db():
    embeddings = OpenAIEmbeddings()
    store = Chroma.from_documents(collection_name="travel", embedding_function=embeddings, persist_directory=PERSIST_DIR) 
    return store
