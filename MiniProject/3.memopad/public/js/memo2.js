...
function saveButton() {
  ...
  const filepath = window.URL.createObjectURL(fileInput);
  console.log(filepath);
  submitBtn.addEventListener("click", async (e) => {
    e.preventDefault();
    const data = await submitMemo(
      titleInput.value,
      textInput.value,
      fileInput.name,
      filepath,
    ...
    data.forEach((item) => {
      ...
      cardDiv.innerHTML = `
          <div class="card-body">
            ...
            <img src="${item.filepath}" class="card-img-top" alt="${item.filename}"></img>
            <p class="card-text"><small class="text-muted">${item.filename}</small></p>
...
async function submitMemo(title, contents) {
  const response = await fetch("/memo", {
    ...
    body: JSON.stringify({ title, contents, filename, fileUrl, filesize }),
  });
